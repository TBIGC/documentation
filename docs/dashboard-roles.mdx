...
Observer users can view the dashboard / action tracker as if they were an admin.
However they do not have permission to modify any data - they can only observe the
data in the dashboard. This user type is useful for giving stakeholders access to
the dashboard but with the expectation that they will not need to change any data.